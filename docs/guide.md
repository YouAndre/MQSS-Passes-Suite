...
Ready to contribute to the Collection of MLIR Passes of the Stack? This guide will help you get
started.

## Installing LLVM/MLIR

Since the Stack compiler is based on LLVM/MLIR infrastructure and CudaQ compiler, your system must
...
\note Do not forget to include `compiler-rt` in the `LLVM_PROJECTS`. This is required by some of the
 Stack MLIR passes. If you do not include it, the project will not compile.

## Initial Setup

1. Fork the Passes repository on GitHub [double blind review].
...
`build.sh` that first builds CudaQ library `cudaq-mlir-runtime`. Then, the script builds the Stack
